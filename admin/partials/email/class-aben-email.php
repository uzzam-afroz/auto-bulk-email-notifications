...
class Aben_Email
{
    ...
    public function aben_email_template()
    {
        ...
        <title></title></head>
        <body>
        <div id="aben-email-template" style="font-family:Open Sans,sans-serif;margin:0;padding:0;background: ' .
            esc_attr($this->body_bg) .
            ';color: #1f2430;">
        <div style="width:100%;max-width:500px;margin: auto;">
        <div style="padding: 50px 30px 30px 30px;">
        <p id ="header-text" style="font-size:16px;display: inline;"><strong>' .
            esc_html($this->header_text) .
            "</strong></p>";
        do_action("aben_after_header_text"); // After Header Text Hook
        echo '<img width="16px" src="' .
            esc_url(ABEN_PLUGIN_URL . "assets/images/hand-emoji.png") .
            '" style="
    margin-left: 5px">
        <p id="header-subtext" style="font-size:16px;">' .
            esc_html($this->header_subtext) .
            "</p>";
        ...
        foreach ($this->posts_to_send as $post) {
            ...
            $excerpt_width =
                $this->show_view_post && $this->is_aben_gw_active() ? 60 : ($this->show_view_post || $this->is_aben_gw_active() ? 85 : 100);
            ...
            if ($this->show_view_post && !empty($image)) {
                echo '<div class="view-post" style="width:15%;margin-right: 25px;align-self:center; margin-top:auto; margin-bottom:auto;"><a href="' .
                    esc_url($link) .
                    '"><img width="100%" max-width="100px" src="' .
                    esc_url($image) .
                    '" alt="' .
                    esc_attr($title) .
                    '" /></a></div>';
            ...
            echo '<div style="width:' .
                esc_attr($excerpt_width) .
                '%;"><p style="font-size:16px;margin:0;color: #008dcd;"><a href="' .
                esc_url($link) .
        ...
        if ($this->show_view_all) {
            echo '<a id="view-all-post" href="' .
                esc_url($this->archive_page_slug) .
                '"style="display:inline-block;padding:15px 0px;background-color:#2271b1;color:#ffffff;text-decoration:none;width: 100%;font-size:16px;">' .
                esc_html($this->view_all_posts_text) .
                "</a>";
        }
        echo '</div></div></div>
        ...
        <a href="' .
            ...
            '"><img src="' .
            esc_url($logo) .
            '" alt="Site Logo" style="max-height:40px; object-fit:contain; margin-top: 10px;"></a><div>';
        ...
        if ($this->show_unsubscribe) {
            echo '<span id="unsubscribe"><a href="' .
                esc_html(home_url("?aben-unsubscribe={{USER_EMAIL}}")) .
                '" style="color:#808080;text-decoration:none">Unsubscribe</a></span>';
        }
        if (!self::is_pro()) {
            echo '</div><p><a href="' .
                esc_url(ABEN_BRAND_LINK) .
                '" style="text-decoration:none;">' .
                esc_html(ABEN_BRAND_TEXT) .
                ' <img src="' .
                esc_url(ABEN_PLUGIN_LOGO) .
                '" width="60px" alt="Aben" style="margin-bottom:-4px"/></a></p>';
