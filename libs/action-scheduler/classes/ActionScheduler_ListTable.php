...
class ActionScheduler_ListTable extends ActionScheduler_Abstract_ListTable
{
    ...
    public function __construct(ActionScheduler_Store $store, ActionScheduler_Logger $logger, ActionScheduler_QueueRunner $runner)
    {
        ...
        $this->sort_by = ["schedule", "hook", "group"];

        $this->search_by = ["hook", "args", "claim_id"];

        $request_status = $this->get_request_status();

        if (empty($request_status)) {
            $this->sort_by[] = "status";
        } elseif (in_array($request_status, ["in-progress", "failed"], true)) {
    ...
    private static function human_interval($interval, $periods_to_include = 2)
    {
        ...
        for (
            $time_period_index = 0, $periods_included = 0, $seconds_remaining = $interval;
            $time_period_index < $num_time_periods && $seconds_remaining > 0 && $periods_included < $periods_to_include;
            $time_period_index++
        ) {
            ...
            if ($periods_in_interval > 0) {
                ...
                $output .= sprintf(
                    translate_nooped_plural(self::$time_periods[$time_period_index]["names"], $periods_in_interval, "action-scheduler"),
                    $periods_in_interval,
                );
    ...
    public function display_admin_notices()
    {
        ...
        if (
            (is_a($this->store, "ActionScheduler_HybridStore") || is_a($this->store, "ActionScheduler_DBStore")) &&
            apply_filters("action_scheduler_enable_recreate_data_store", true)
        ) {
            $table_list = ["actionscheduler_actions", "actionscheduler_logs", "actionscheduler_groups", "actionscheduler_claims"];
        ...
        if ($this->runner->has_maximum_concurrent_batches()) {
            ...
            if (false === $async_request_lock_expiration || $async_request_lock_expiration < time()) {
                ...
                $async_request_message = sprintf(
                    __('A new queue has begun processing. <a href="%s">View actions in-progress &raquo;</a>', "action-scheduler"),
                    esc_url($in_progress_url),
                );
            } else {
                /* translators: %d: seconds */
                $async_request_message = sprintf(
                    __("The next queue will begin processing in approximately %d seconds.", "action-scheduler"),
                    $async_request_lock_expiration - time(),
                );
            }
        ...
        if (is_array($notification)) {
            ...
            if (1 === absint($notification["success"])) {
                ...
                $action_message_html = sprintf(
                    __('Could not process change for action: "%1$s" (ID: %2$d). Error: %3$s', "action-scheduler"),
                    $action_hook_html,
                    esc_html($notification["action_id"]),
                    esc_html($notification["error_message"]),
                );
            }
